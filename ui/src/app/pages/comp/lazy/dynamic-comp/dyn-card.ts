...
@Component({
  ...
  template: `
    <nz-card class="b-r-8" style="border-width: 2px;" [nzBodyStyle]="{ padding: '20px' }" [style.border-color]="themeStyle().color">
      ...
      <button nz-button [style.border-color]="themeStyle().color" [style.color]="themeStyle().color" (click)="destroy.emit()"> 销毁此组件 </button>
    </nz-card>
  `
})
